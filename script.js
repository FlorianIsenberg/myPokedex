...
function generatePokecard(bigPokemon, fightclasstype, i) {
  return `
    ...
    <div class="openPokemonCardIMG">
      <img class="arrowleft" onclick="slide(${
        i - 1
      })" src="img/arrow_back.svg" alt="">
      <img class="pokemonimg" src="${
        bigPokemon.sprites.other.dream_world.front_default
      }" alt=""/>
      <img class="arrowright" onclick="slide(${
        i + 1
      })" src="img/arrow_forward.svg" alt="">
    </div>
  </div>
...
function searchPokemon() {
  ...
  for (let s = 0; s < allPokemon.length; s++) {
    ...
    if (pokemon.name.includes(search)) showPokemon(pokemon, s);
  }
}
